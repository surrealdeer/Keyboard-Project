...
namespace Keyboard
{
    public class KeyboardManager : MonoBehaviour
    {
        ...
        [SerializeField] private TMP_InputField[] outputFields;
        [SerializeField] private TMP_InputField adInputField;
        [SerializeField] private TMP_InputField soyadInputField;
        [SerializeField] private TMP_InputField sicilNoInputField;

        [SerializeField] private Button girisButton;
        ...
        private void Awake()
        {
            ...
            outputFields = new TMP_InputField[3]; 

            outputFields[0] = adInputField;
            outputFields[1] = soyadInputField;
            outputFields[2] = sicilNoInputField;
        ...
        private void KeyPress(string key)
        {
            ...
            foreach (TMP_InputField outputField in outputFields)
            {
        ...
        private void OnDeletePress()
        {
            foreach (TMP_InputField outputField in outputFields)
            {
                if (string.IsNullOrEmpty(outputField.text)) continue;
        ...
        {
            bool anyFieldExceedsMaxLength = false;

            foreach (TMP_InputField field in outputFields)
            {
                if (field.text.Length > maxCharacters)
                {
                    anyFieldExceedsMaxLength = true;
                    break;
                }
            }

            // Raise event to enable or disable keys based on the text length
            bool keysEnabled = !anyFieldExceedsMaxLength;
            keyChannel.RaiseKeysStateChangeEvent(keysEnabled);

            // Disables or enables the enter button based on the text length
            bool allFieldsMeetMinLength = outputFields.All(field => field.text.Length >= minCharacters);
            enterButton.interactable = allFieldsMeetMinLength;
        ...
        public void OnEnterButtonClicked()
        {
            foreach (TMP_InputField field in outputFields)
            {
                Debug.Log("Input Field Text: " + field.text);
            }

            PlayerPrefs.SetString("PlayerName", adInputField.text);
            PlayerPrefs.SetString("PlayerSurname", soyadInputField.text);
            PlayerPrefs.SetString("PlayerID", sicilNoInputField.text);
